...
resource "aws_nat_gateway" "nat" {
  ...
  depends_on    = ["aws_internet_gateway.wix-staging-ig"]
}
...
resource "aws_subnet" "wix-staging-pub-subnet" {
    vpc_id                  = "${aws_vpc.wix-staging.id}"
    cidr_block              = "10.100.1.0/24"
    availability_zone       = "us-west-2b"
    map_public_ip_on_launch = false
    tags {
        "Name" = "wix-staging pub subnet"
    }
}
resource "aws_subnet" "wix-packer" {
    vpc_id                  = "${aws_vpc.wix-staging.id}"
...
resource "null_resource" "staging-instance" {
    ...
    connection={
        host="${aws_instance.staging-instance.*.private_ip[count.index]}"
        user="admin"
    ...
    provisioner "local-exec" {
        command = "echo ${aws_instance.staging-instance.*.private_ip[count.index]} node-${format("%02d", count.index)}.staging.wixpress.com"
    }
...
resource "aws_route_table" "wix-staging-pub" {
    vpc_id     = "${aws_vpc.wix-staging.id}"
    ...
    tags {
        "Name" = "wix-staging pub routing table"
    }
}
resource "aws_main_route_table_association" "wix-stagingdefault-route" {
    route_table_id = "${aws_route_table.wix-staging-rt.id}"
    vpc_id = "${aws_vpc.wix-staging.id}"
}
resource "aws_route_table_association" "wix-staging-pub" {
